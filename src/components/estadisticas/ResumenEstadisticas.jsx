...
export const StatCard = ({ title, value, icon: Icon, color = 'orange', trend }) => {
  const colorClasses = {
    ...
    <div className="stat-card group relative bg-white bg-opacity-80 rounded-xl border p-5 shadow">
      <div className="flex items-center justify-between mb-4">
        ...
        {trend !== undefined && trend !== null && (
          <div className={`text-xs font-semibold px-2 py-1 rounded-full ${parseFloat(trend) > 0 ? 'bg-green-100 text-green-600' : parseFloat(trend) < 0 ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-600'
            }`}>
