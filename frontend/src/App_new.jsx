...
const Dashboard = () => {
  ...
  const comparisonData = useMemo(() => {
    ...
    Object.keys(datasets).forEach(key => {
      const data = getActiveData === 'employment' ? employmentData : 
                   getActiveData === 'unemployment' ? unemploymentData :
                   getActiveData === 'informal' ? informalEmploymentData :
                   getActiveData === 'laborForce' ? laborForceData : salaryData;
      
      // Get latest year data for each country for this dataset
      selectedCountries.forEach(country => {
        const countryData = data.filter(row => row.country.includes(country));
        if (countryData.length > 0) {
          const latest = countryData.sort((a, b) => b.year - a.year)[0];
          if (!latestYearData[country]) latestYearData[country] = {};
          latestYearData[country][key] = latest.value;
        }
      });
    });
